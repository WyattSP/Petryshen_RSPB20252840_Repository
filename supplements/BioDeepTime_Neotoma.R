...
source("../BioDeepTime_Haar/src/Haar.R")

# Main loop functions
neotoma_haar_analysis_mc <- function(series_list, scaling_factor = 1000, ncores = 10){
  ...
  mclapply(1:iter_l, function(i){
    ...
    data_series <- bdt %>%
      ...
      mutate(tax = word(analyzedTaxon, 1))
    ...
    # Save result to JSON
    save_dir <- "../BioDeepTime_Haar/Neotoma_Results"
...
series_lengths <- readRDS("../BioDeepTime_Haar/src/series_iter_BDT_FIG.rds")
...
#  Run  #
#########
# Filter for only Neotoma
neotoma <- l_series %>% filter(db == "Neotoma")
# Run Analysis
neotoma_haar_analysis_mc(neotoma)
